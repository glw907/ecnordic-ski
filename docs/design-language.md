...
| **Body (sitewide standard)** | Alegreya · **0.92rem** | 400 | body | 1.6 | 0 |
| Supporting / grid-cell prose | Alegreya · 0.85rem | 400 | **body-soft** | 1.5 | 0 |
| Term / mini-label | Nunito · 1.0rem | 700 | heading | 1.3 | 0 |
| Eyebrow / label | Nunito · 0.68–0.8rem | 700 | muted (or role) | n/a | ramp ↓ |
| Meta / caption | Alegreya · 0.72rem | 400 | muted / faint | 1.3 | 0 |

**Body is one size, site-wide.** The standard body size (**0.92rem**) is set once
on `.post-body` in `src/app.css` and applies everywhere by default: prose, list
items, posts, and card content alike. DaisyUI's `.card-body` ships its own 0.875rem,
so it's overridden to *inherit* the standard
...
clear of `muted`'s caption weight. `muted` stays reserved for secondary
...
rather than blinking in. Keep it subtle: felt, not watched.
...
**neutral heading color, not the role color** (amber dark enough to read as text
turns muddy/brown, so the role color stays in the *icon* and link underline
only). A faint role-tinted wash alone sets it apart from the card stack, with
**no border, no shadow**, so it recedes as an aside instead of floating like a
module. Padding matches `.card-body` (1.5rem) and the icon sits inside the label
rather than in a left column, so the label and prose align with the card text
above/below, no left-edge shift in the stack.
...
| Short titled points that are peers, not a sequence | **Grid card** (`.ec-grid`) | Two unnumbered columns, parallel and scannable. |
...
|---|---|---|
| **Person / profile card** | Photo + name + role + short blurb, in a responsive grid | The **Volunteers** roster is the textbook use case |
...
glyphs are truly universal. An icon is never the message; it *accelerates*
...
**Skip the icon when** any of these is true (use none):
...
3. **Apply the type scale.** Every text level separated on size + weight +
   ...
   Nunito 700 + the tracking ramp; `muted` only for secondary text
